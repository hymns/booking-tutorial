                        ...
                        <form class="row g-3" action="{{ route('items.update', $item->id) }}" method="post">
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-control @error('type_id') is-invalid @enderror" name="type_id">
                                    ...
                                    <option value="{{ $row->id }}" {{ (($row->id == old('type_id') || $row->id == $item->type_id) ? 'selected' : '') }}>{{ $row->name }}</option>
                                    @endforeach
                                </select>
                                @error('type_id')
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea rows="5" class="form-control @error('name') is-invalid @enderror" id="description" name="description">{{ old('description') ?? $item->description }}</textarea>
                                @error('description')
                                <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
